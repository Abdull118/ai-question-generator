...
const Page = () => {
    ...
    const fetchQuestions = async () => {
      ...
      try {
        const response = await fetch('/api/getQuestions', {
          ...
          body: JSON.stringify({ collectionName: `MOD 5 pharm` }), // Pass the collection name dynamically
        });
    ...
    if (questions.length > 0) {
      ...
      if (selectedOption === currentQuestion.answer) {
        ...
        const incorrectQuestionWithSubject = {
          ...currentQuestion,
          subject: 'mod5pharm',  // Add the subject field here
        };
  ...
  const totalQuestions = reviewMode 
    ? incorrectQuestions.filter(q => q.subject === 'mod5pharm').length // Filter incorrect questions based on subject
    : questions.length;

  const filteredIncorrectQuestions = reviewMode
    ? incorrectQuestions.filter(q => q.subject === 'mod5pharm') // Only include questions with subject 'mod5pharm'
    : [];
