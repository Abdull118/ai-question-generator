...
const Page = () => {
    ...
    const fetchQuestions = async () => {
      ...
      try {
        const response = await fetch('/api/getQuestions', {
          ...
          body: JSON.stringify({ collectionName: `MOD 5 MMI` }), // Pass the collection name dynamically
        });
    ...
    if (questions.length > 0) {
      ...
      if (selectedOption === currentQuestion.answer) {
        ...
        const incorrectQuestionWithSubject = {
          ...currentQuestion,
          subject: 'mod5mmi',  // Add the subject field here
        };
  ...
  const totalQuestions = reviewMode 
    ? incorrectQuestions.filter(q => q.subject === 'mod5mmi').length // Filter incorrect questions based on subject
    : questions.length;

  const filteredIncorrectQuestions = reviewMode
    ? incorrectQuestions.filter(q => q.subject === 'mod5mmi') // Only include questions with subject 'mod5mmi'
    : [];
