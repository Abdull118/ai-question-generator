  ...
  const fetchQuestions = async () => {
    try {
      ...
      const response = await fetch('/api/getQuestions', {
        ...
        body: JSON.stringify({
          collectionName: `mod6_path${selectedLecture}.txt`, 
          folder: `mod6`, // Assuming folders are named modX
          fileName: `path${selectedLecture}.txt`, // Assuming files follow this pattern
        }),
      });
    ...
    try {
      ...
      const response = await fetch('/api/getIncorrectQuestions', {
        ...
        body: JSON.stringify({
          collectionName: `mod6_path${selectedLecture}.txt`, 
          folder: `mod6`,
          fileName: `path${selectedLecture}.txt`,
          ipAddress: ipAddress
        }),
      });
    ...
    try {
      ...
      await fetch('/api/AI', {
        ...
        body: JSON.stringify({
          ...
          sampleQuestions: `
              1: A 25 y.o. male presents to his physician with a complaint of “yellow eyes” for the past day. For the past five days he has been ill with a low-grade fever, rhinorrhea, myalgias, and generalized malaise. The physical exam confirms scleral icterus but is otherwise unremarkable. Electrolytes and CBC are all within normal limits. Other labs show: AST 31IU/L, ALT 25 IU/L, AlkPO4 45 IU/L, Total Bilirubin 3 mg/dL, LDH 40IU/L, Haptoglobin 76 mg/dl (nl 46-316). His UA demonstrates a normal bilirubin level. What is the most appropriate treatment for this patient?
                A. Corticosteroids
              ...
              2: A 65-year-old female presents to the ED with persistent RUQ pain with nausea and vomiting. CT of the abdomen reveals a polypoid mass of the gallbladder protruding into the lumen, diffuse thickening of the gallbladder wall and enlarged lymph nodes. This patient most likely has a history of which of the following?
